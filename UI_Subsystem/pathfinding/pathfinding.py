...
def scaleGrid(grid_point, dimension = dimension, input = "var", output = "GPS"):
    point1 = (30.6203464,-96.3411845) #(30.6222648,-96.3389598) #COORDINATES FOR POLO
    point2 = (30.6221237,-96.3395430) #(30.6310495,-96.4042106) 
...
def getTurn(instruction, current_orientation, next_orientation):
    if ((current_orientation[1] == -1*next_orientation[1]) and (current_orientation[1] != 0)) or ((current_orientation[0] == -1*next_orientation[0]) and (current_orientation[0] != 0)):
            angle = 180
...
def getFinalOrientation(p1, p2):
    prev = scaleGrid(p1, input = "var", output = "point")
    last = scaleGrid(p2, input = "var", output = "point")
    if prev == last:
        print("getFinalOrientation() Error: both inputs are the same. Assuming North face.")
        return 90
    if(prev[1] - last[1]) > 0:
        return 270 #S
    elif(prev[1] - last[1]) < 0:
        return 90 #N
    elif(prev[0] - last[0]) < 0:
        return 0 #E
    elif(prev[0] - last[0]) > 0:
        return 180 #S
    return

def instructionset(pathGPS, turn = 0):
    #orientation represented as unit circle. If a rover is to move forward,
    #then it is assumed to be moving at 90 degrees
    instructions = ["F$0"]
    if turn == 90:
        current_orientation = [1,0] #E
    elif turn == 180:
        current_orientation = [0,-1] #S
    elif turn == 270:
        current_orientation = [-1,0] #W
    elif turn == 0:
        current_orientation = [0,1] #N
    
    #convert path input to point
    if len(pathGPS) == 0:
        instructions = ["F$0","T$-90","T$-90","T$-90","T$-90","F$0"]
        return instructions
...
def addObstacle(rover_coordinate, orientation, obstacle_type, center_point, area):
    ...
    i = 2 #INCREASE IF IT DOESN'T AVOID ENOUGH
    for j in range(i,-i-1,-1):
        ...
        for k in range(i+2,-i-1-2,-1):
            #check if rover is going to be covered by obstacle or not
            if (obstacle_node[0]+k,obstacle_node[1]+j) == rover_node:
                break
            else:
...
def tempAddObstacle(current_point):
    gps_point = (current_point[0]+0.0001,current_point[1])
...
def avoidCone():
    instructions = ["F$0","T$90","F$0.9144","T$-90","F$3","T$-90","F$0.9144","T$90", "F$1.1", "T$-90", "T$-90", "T$-90", "T$-90", "F$0"]
    return instructions
...
def findOrientation(gps_point, path):
    ...
    init = scaleGrid(gps_point, input = "var", output = "point")
    node_list = scaleGrid(path, input = "list", output = "point")
    current_node = (0,0)
    next_node = (0,0)
    tol = 1
    print("finding closest node point...")
    '''while (node_point not in node_list) or (tol >= 20):
