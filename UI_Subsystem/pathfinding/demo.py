...
#rover setup and waypoint calc
rover1_start = (20,2) #changeable
rover2_start = (14,4) #changeable
rover_coords = [rover1_start, rover2_start]
points_of_interest = [(1,14),(5,14),(3,2),(6,28),(7,19),
                      (7,27),(16,2),(30,30),(38,12),(35,6),(32,5)] #changeable
temp1, temp2 = bs.roverBoundaries(rover1_start,rover2_start,points_of_interest)
rover1_list = [rover1_start, temp1]
#rover1_list += temp1
rover2_list = [rover2_start, temp2]
#rover2_list += temp2
...
rover2_change = True
pf.draw_grid(diagram) #draws diagram
...
def list2csv(list, filename):
    with open("./path_folder/"+filename+".csv", 'w', newline='') as myfile:
...
#output csv for points of interest for each rover
list2csv(POI_gps1, "rover1poi")
list2csv(POI_gps2, "rover2poi")

#MAIN OPERATION
userinput = input("Press enter to begin operation.") #buffer to start
while operation == True: #fix this??
    #iterate through the lists to create path and instructions
    if i < len(rover1_list)-1 and rover1_change == True: #first list
        path1_gps = []
    ...
    if j < len(rover2_list)-1 and rover2_change == True: #second list
        path2_gps = []
    ...
    #FUNCTIONS FOR ROVER OBST DETECTION
    ...
    if userinput == "c": #rover 1 detection
        ...
        if len(path1) != 2:
            ...
            obs_center = path1[1]
        #create obstacle object in diagram
